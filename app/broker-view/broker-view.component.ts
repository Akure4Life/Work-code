...
export class BrokerViewComponent implements OnInit {
  ...
  getClaim() {
    ...
    .subscribe(
      response => {
        ...
        if (this.BrokerFirstViewDateTimeUTC == null) {
          this.patchClaimRecord.status = "WithBroker"
  ...
  submitClaim(): void {
    this.patchClaimRecord.brokerContact = this.BrokerContact;
    this.patchClaimRecord.brokerPhone = this.BrokerPhone;
    this.patchClaimRecord.lossName = this.LossName;
    this.patchClaimRecord.amountOutstandingGBP = this.AmountOutstandingGBP;
    ...
    this.patchClaimRecord._TextToPrependToExistingComments = this.BrokerComments;
      ...
      response => {
        ...
        if (this.responseStatus == 200) {
          ...
          .subscribe(
            response => {
              this.BrokerFirstViewDateTimeUTC = response.json().brokerFirstViewDateTimeUTC;
              this.BrokerFirstAssignmentToCarrierDateTimeUTC = response.json().brokerFirstAssignmentToCarrierDateTimeUTC;
            });
          }
        }
      );
  ...
  isSaveDisabled(form: NgForm): boolean {
    ...
    if (!((form.valid) && (this.Status == "Broker"))) {
  ...
  isSubmitDisabled(form: NgForm): boolean {
    if (!((form.valid) && (this.Status == "Carrier" || this.Status == "Claimant"))) {
